...
if ( ! function_exists( 'ou_degree_format_post_data' ) ) {
	 ...
	 * @param  int|null $existing_id The ID of the existing post if it exists, null if not
	 * @return array
	 */
	function ou_degree_format_post_data( $meta, $program, $existing_id ) {
		...
		foreach( $existing_meta as $meta_key => $meta_value ) {
			if ( substr( $meta_key, 0, $prefix_length ) === 'degree_lock_' ) {
				$locks[substr( $meta_key, $prefix_length )] = $meta_value[0];
			}
		}

		if ( ! ou_degree_meta_can_import( $locks, 'tuition' ) ) {
			unset( $meta['degree_resident_tuition'] );
			unset( $meta['degree_nonresident_tuition'] );
		}

		if ( ! ou_degree_meta_can_import( $locks, 'short_name' ) ) {
			unset( $meta['degree_short_name'] );
		}

		if ( ! ou_degree_meta_can_import( $locks, 'pdf' ) ) {
			unset( $meta['degree_pdf'] );
		}
		...
		if ( ou_degree_meta_can_import( $locks, 'outcomes' ) ) {
		...
		if ( ou_degree_meta_can_import( $locks, 'projections' ) ) {
		...
		if ( ou_degree_meta_can_import( $locks, 'deadlines' ) ) {
	...
	add_filter( 'ucf_degree_get_post_metadata', 'ou_degree_format_post_data', 10, 3 );
}

if ( ! function_exists( 'ou_degree_meta_can_import' ) ) {
	/**
	 ...
	 * @return bool
	 */
	function ou_degree_meta_can_import( $locks, $key ) {
		if ( ! array_key_exists( $key, $locks ) ) return true;

		return $locks[$key] === "0" ? true : false;
	}
}
